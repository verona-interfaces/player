# Verona Interfaces Specification "Player" 5.2.0 documentation
...
| playerConfig.pagingMode | string | If the player makes it optional, then the page presentation can be changed by the host. The pages are presented separately, with page navigation buttons, concatenated to one big page or concatenated in snap mode (scrolling vertically, but only one page is visible at a time). | allowed (`"separate"`, `"buttons"`, `"concat-scroll"`, `"concat-scroll-snap"`) | - | - |

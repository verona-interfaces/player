
    const schema = {
  "asyncapi": "2.5.0",
  "info": {
    "title": "Verona Interfaces Specification \"Player\"",
    "version": "6.1.1",
    "description": "This is one part of the [Verona Online Assessment Standards](https://verona-interfaces.github.io). All messages are sent via the postMessage function of the html page. The player takes the page root of its parent as target (parent.window), and the application binds the function call to the iframe element of the player.\n\nMost important, the message body carries as first parameter the operationId of the message.\n\nThe html page MUST contain a script-tag with metadata. The syntax and structure of this data are described [here](https://verona-interfaces.github.io/intro/metadata.html).\n",
    "license": {
      "name": "CC0-1.0",
      "url": "http://creativecommons.org/publicdomain/zero/1.0/"
    },
    "contact": {
      "name": "Institute for Educational Quality Improvement (IQB)",
      "url": "https://www.iqb.hu-berlin.de",
      "email": "iqb-tbadev@hu-berlin.de"
    }
  },
  "channels": {
    "vopReadyNotification": {
      "subscribe": {
        "operationId": "vopReadyNotification",
        "summary": "Ready Notification",
        "description": "The player announces that it's code is loaded and initialized so the communication can start.",
        "message": {
          "name": "vopReadyNotification",
          "payload": {
            "type": "object",
            "properties": {
              "metadata": {
                "type": "string",
                "description": "Via this property, the player sends the stringified metadata object defined as json-ld in the header of the html file. See [here](https://github.com/verona-interfaces/metadata/#readme) for more information.",
                "x-parser-schema-id": "<anonymous-schema-2>"
              }
            },
            "required": [
              "metadata"
            ],
            "x-parser-schema-id": "<anonymous-schema-1>"
          }
        }
      }
    },
    "vopStartCommand": {
      "publish": {
        "operationId": "vopStartCommand",
        "summary": "Start Command",
        "description": "The application sends parameters for running the unit and commands the start of user interaction.",
        "message": {
          "name": "vopStartCommand",
          "payload": {
            "type": "object",
            "properties": {
              "sessionId": {
                "description": "The session id flags all communication. If a message has no or empty session id, it's not processed. The session id is unique and was generated by the application. Any simple algorithm would work. The session id helps to link the correct unit with the data of the message. Using the id of the player hosting html element is less reliable, because the element could be reused with another unit.",
                "type": "string",
                "example": "idk8ur5jf9ru5jk",
                "x-parser-schema-id": "SessionIdString"
              },
              "unitDefinition": {
                "type": "string",
                "format": "byte",
                "description": "The definition of the unit (if given) lets the player adapt. An audio player gets it's audio sequence, a choice player gets it's options...",
                "example": "iqb-scripted::1.4.0 title::Testscript Title2??Hilfetext2 multiple-choice::mc_var1::1::Multiple Choice Feld: ::Choice1##Choice2##Choice3??Hilfetext1 drop-down::dd_var1::1::Dropdown Feld: ::Choice1##Choice2##Choice3??Hilfetext1 checkbox::check_var1::0::Ja klick mal! if-start::dd_var1::1 input-text::jajaj::sap osjxapsoxa if-else input-text::jajaj22::sap osjxapsoxa UUUU text::NOT Choice1 if-end",
                "x-parser-schema-id": "<anonymous-schema-4>"
              },
              "unitDefinitionType": {
                "type": "string",
                "description": "This lets the player the unit definition format know. This might avoid UI mess after getting old definitions.",
                "example": "iqb-scripted@1.4.0",
                "x-parser-schema-id": "<anonymous-schema-5>"
              },
              "unitState": {
                "type": "object",
                "description": "These data stand for the answers and some additional information on the status of responding. Emitted by the player during the test, it is given back to the player on start to restore the former state.",
                "properties": {
                  "dataParts": {
                    "description": "This data is used by the player to restore the former response state and by data processing systems to analyse the responses. The host must buffer all data parts, because the player might send only changed data parts, not always the whole package. The host stores all parts but only the last given version (respect timeStamp!). Every data part is identified by a unique key, the data is stored as serialized object (string).",
                    "type": "object",
                    "additionalProperties": {
                      "type": "string",
                      "format": "byte",
                      "x-parser-schema-id": "<anonymous-schema-7>"
                    },
                    "example": {
                      "page1": "{\"a\": 1, \"b\": 233}",
                      "page2": "{\"c\": \"sehr gut!\", \"d\": true}"
                    },
                    "x-parser-schema-id": "<anonymous-schema-6>"
                  },
                  "presentationProgress": {
                    "description": "This reports the progress of presentation of unit content. The host can enable or disable the navigation to the next unit depending on that value. For example, all audio elements are required to be played or all pages must be presented to the page bottom. This way, giving no response to an item is always intended.",
                    "type": "string",
                    "enum": [
                      "none",
                      "some",
                      "complete"
                    ],
                    "example": "none",
                    "x-parser-schema-id": "<anonymous-schema-8>"
                  },
                  "responseProgress": {
                    "description": "This reports the progress of responding. The host can enable or disable the navigation to the next unit depending on that value. The value 'complete' announces not only that all required responses are given but that these responses are valid too. The value 'complete' can be sent even not all responses are given - the host is interested only in REQUIRED responses. So make sure to mark all response elements correctly corresponding to this behaviour.",
                    "type": "string",
                    "enum": [
                      "none",
                      "some",
                      "complete"
                    ],
                    "example": "some",
                    "x-parser-schema-id": "<anonymous-schema-9>"
                  },
                  "unitStateDataType": {
                    "type": "string",
                    "description": "This string specifies the format of the data stored in dataParts (value). Every transformation or analysis of stored unit data requires knowledge about the format. When the host sends data to the player to restore the former unit state, the player should check the given data type to avoid data mess after getting an old data type.",
                    "example": "iqb-standard@2.1.2",
                    "x-parser-schema-id": "<anonymous-schema-10>"
                  }
                },
                "x-parser-schema-id": "UnitState"
              },
              "playerConfig": {
                "type": "object",
                "description": "This data supplies some information or instruction about this specific run of the unit (number, unit title, some behavioral data for the player).",
                "properties": {
                  "unitNumber": {
                    "description": "The player might show the numbering of the current unit to ease the navigation.",
                    "type": "integer",
                    "minimum": 1,
                    "example": "14",
                    "x-parser-schema-id": "<anonymous-schema-11>"
                  },
                  "unitTitle": {
                    "description": "The player might show the title of the current unit. Unless the unit definition could consist of a title, the host might decide to change the title. For example, this is necessary if one unit appears more then once in a booklet.",
                    "type": "string",
                    "maxLength": 50,
                    "example": "Ein wunderbarer Ausflug",
                    "x-parser-schema-id": "<anonymous-schema-12>"
                  },
                  "unitId": {
                    "description": "The player might include the internal unit id in state variables or logs.",
                    "type": "string",
                    "maxLength": 20,
                    "example": "M24093EX",
                    "x-parser-schema-id": "<anonymous-schema-13>"
                  },
                  "logPolicy": {
                    "description": "The host expects the player to send no logs, only important logs, all possible logs or even logs for debugging purposes. This is a guideline. The player can decide what exactly the logging consists of.",
                    "type": "string",
                    "enum": [
                      "disabled",
                      "lean",
                      "rich",
                      "debug"
                    ],
                    "example": "lean",
                    "x-parser-schema-id": "<anonymous-schema-14>"
                  },
                  "pagingMode": {
                    "description": "If the player makes it optional, then the page presentation can be changed by the host. The pages are presented separately, with page navigation buttons, concatenated to one big page or concatenated in snap mode (scrolling vertically, but only one page is visible at a time).",
                    "type": "string",
                    "enum": [
                      "separate",
                      "buttons",
                      "concat-scroll",
                      "concat-scroll-snap"
                    ],
                    "example": "concat-scroll",
                    "x-parser-schema-id": "<anonymous-schema-15>"
                  },
                  "printMode": {
                    "description": "If this mode is 'on', the host presents the unit as preview for printing. Depending on the unit, the player modifies the presentation by (1) assignment of print styles, (2) ALL unit parts are displayed regardless of conditions, (3) all pages are displayed among each other, (4) the height of the given space is ignored (no vertical scrollbars by player). If the mode is set to 'on-with-ids', the player adds control labels to show the IDs.",
                    "type": "string",
                    "enum": [
                      "off",
                      "on",
                      "on-with-ids"
                    ],
                    "example": "off",
                    "x-parser-schema-id": "<anonymous-schema-16>"
                  },
                  "enabledNavigationTargets": {
                    "description": "This lets the player know, what navigation target is enabled to natigate to. The player might then alter the presentation of responding buttons.",
                    "type": "array",
                    "items": {
                      "description": "This enumeration lists all possible targets the player could send a navigation request for, relative to the position of the current unit. This could be the next and previous unit, last and first unit of the current range of units (booklet, testlet) or the end of the test. The ending is seen as kind of termination of test, e. g. the announcement of the testee, that all responses are given. What exactly the host will navigate to depends of the nature of the test or the use case and might depend on configuration parameters of the specific booklet as well.",
                      "type": "string",
                      "enum": [
                        "next",
                        "previous",
                        "first",
                        "last",
                        "end"
                      ],
                      "example": "next",
                      "x-parser-schema-id": "NavigationTarget"
                    },
                    "x-parser-schema-id": "<anonymous-schema-17>"
                  },
                  "startPage": {
                    "description": "This requests the player to navigate to a certain page after loading. The host might know this page id from former usages of the unit.",
                    "type": "string",
                    "example": "page5",
                    "x-parser-schema-id": "<anonymous-schema-18>"
                  },
                  "directDownloadUrl": {
                    "description": "After starting the player and loading the unit definition and former responses, it might be necessary to load additional code or data from the server. This data is identified by an resource ID (usually a file name). The player can download this resource by itself without further interaction with the host frontend. The property directDownloadUrl provides the url for download. The player extends this url by an url separator \"/\" and the resource ID (uri-encoded if needed).",
                    "type": "string",
                    "example": "https://www.iqb-testcenter.de/download/iskeid-34e845-didmmemdkek",
                    "x-parser-schema-id": "<anonymous-schema-19>"
                  },
                  "sharedParameters": {
                    "type": "array",
                    "items": {
                      "type": "object",
                      "description": "This object structure is used to share data between instances of the module. The host may collect all shared data sent by modules and provide it to every module.",
                      "properties": {
                        "key": {
                          "description": "Identifier to find or set the shared parameter.",
                          "type": "string",
                          "minLength": 2,
                          "x-parser-schema-id": "<anonymous-schema-21>"
                        },
                        "value": {
                          "description": "Value of the parameter.",
                          "type": "string",
                          "x-parser-schema-id": "<anonymous-schema-22>"
                        }
                      },
                      "required": [
                        "key"
                      ],
                      "example": {
                        "key": "KEYBOARD_USE",
                        "value": "INTERNAL_ONLY"
                      },
                      "x-parser-schema-id": "SharedParameter"
                    },
                    "x-parser-schema-id": "<anonymous-schema-20>"
                  }
                },
                "x-parser-schema-id": "PlayerConfig"
              }
            },
            "required": [
              "sessionId"
            ],
            "x-parser-schema-id": "<anonymous-schema-3>"
          }
        }
      }
    },
    "vopStateChangedNotification": {
      "subscribe": {
        "operationId": "vopStateChangedNotification",
        "summary": "State Changed Notification",
        "description": "The state of the unit and/or of the player has changed. Some logs entries might be sent.",
        "message": {
          "name": "vopStateChangedNotification",
          "payload": {
            "type": "object",
            "properties": {
              "sessionId": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.sessionId",
              "timeStamp": {
                "type": "string",
                "format": "date-time",
                "description": "Ensures, that later arriving states are ignored.",
                "example": "2021-11-15T10:22:25Z",
                "x-parser-schema-id": "<anonymous-schema-24>"
              },
              "unitState": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.unitState",
              "playerState": {
                "type": "object",
                "description": "The player sends some information on pages (valid pages, current page).",
                "properties": {
                  "validPages": {
                    "description": "This is an array with the id of the page und its label. The order of the items matter.",
                    "type": "array",
                    "items": {
                      "type": "object",
                      "properties": {
                        "id": {
                          "type": "string",
                          "x-parser-schema-id": "<anonymous-schema-27>"
                        },
                        "label": {
                          "type": "string",
                          "x-parser-schema-id": "<anonymous-schema-28>"
                        }
                      },
                      "required": [
                        "id"
                      ],
                      "x-parser-schema-id": "<anonymous-schema-26>"
                    },
                    "example": [
                      {
                        "id": "p1",
                        "label": "start"
                      },
                      {
                        "id": "p3",
                        "label": "on the beach"
                      }
                    ],
                    "x-parser-schema-id": "<anonymous-schema-25>"
                  },
                  "currentPage": {
                    "description": "Id of the page currently presented. This id is taken from the list of valid pages. If the paging mode is 'concat-scroll', the first page with parts in view port is taken as current page.",
                    "type": "string",
                    "example": "p3",
                    "x-parser-schema-id": "<anonymous-schema-29>"
                  },
                  "sharedParameters": {
                    "type": "array",
                    "items": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.playerConfig.properties.sharedParameters.items",
                    "x-parser-schema-id": "<anonymous-schema-30>"
                  }
                },
                "x-parser-schema-id": "PlayerState"
              },
              "log": {
                "type": "array",
                "description": "Log entries (lean version) add some information to the response in order to understand the response process better or (rich version) let the analyst replay every change of the state.",
                "items": {
                  "description": "The player can send any information about events happening during the interaction phase.",
                  "type": "object",
                  "properties": {
                    "timeStamp": {
                      "type": "string",
                      "format": "date-time",
                      "example": "2021-11-15T10:22:25Z",
                      "x-parser-schema-id": "<anonymous-schema-32>"
                    },
                    "key": {
                      "type": "string",
                      "description": "This key might help to classify the events afterwards.",
                      "example": "UNIT_NAVIGATION_FAILED",
                      "x-parser-schema-id": "<anonymous-schema-33>"
                    },
                    "content": {
                      "type": "string",
                      "format": "byte",
                      "description": "Some information to specify the event.",
                      "example": "{\"unitId\": \"M24093EX\", \"reason\": \"responsesIncomplete\"}",
                      "x-parser-schema-id": "<anonymous-schema-34>"
                    }
                  },
                  "required": [
                    "timeStamp",
                    "key"
                  ],
                  "x-parser-schema-id": "LogEntry"
                },
                "x-parser-schema-id": "<anonymous-schema-31>"
              }
            },
            "required": [
              "sessionId",
              "timeStamp"
            ],
            "x-parser-schema-id": "<anonymous-schema-23>"
          }
        }
      }
    },
    "vopPageNavigationCommand": {
      "publish": {
        "operationId": "vopPageNavigationCommand",
        "summary": "Page Navigation Command",
        "description": "The host wants the player to navigate to a specific page.",
        "message": {
          "name": "vopPageNavigationCommand",
          "payload": {
            "type": "object",
            "properties": {
              "sessionId": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.sessionId",
              "target": {
                "type": "string",
                "description": "Id of the page matching one of the validPages given to the application by the playerState data of the StateChangedNotification.",
                "example": "page4",
                "x-parser-schema-id": "<anonymous-schema-36>"
              }
            },
            "required": [
              "sessionId",
              "target"
            ],
            "x-parser-schema-id": "<anonymous-schema-35>"
          }
        }
      }
    },
    "vopUnitNavigationRequestedNotification": {
      "subscribe": {
        "operationId": "vopUnitNavigationRequestedNotification",
        "summary": "Unit Navigation Requested Notification",
        "description": "The user has triggered a navigation request.",
        "message": {
          "name": "vopUnitNavigationRequestedNotification",
          "payload": {
            "type": "object",
            "properties": {
              "sessionId": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.sessionId",
              "target": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.playerConfig.properties.enabledNavigationTargets.items"
            },
            "required": [
              "sessionId",
              "target"
            ],
            "x-parser-schema-id": "<anonymous-schema-37>"
          }
        }
      }
    },
    "vopNavigationDeniedNotification": {
      "publish": {
        "operationId": "vopNavigationDeniedNotification",
        "summary": "Navigation Denied Notification",
        "description": "The host lets the player know that a navigation request was denied due to player state.",
        "message": {
          "name": "vopNavigationDeniedNotification",
          "payload": {
            "type": "object",
            "properties": {
              "sessionId": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.sessionId",
              "reason": {
                "type": "array",
                "description": "This optional parameter explains the reason for the failed attempt to navigate. The player can visualise the problem to the testee. For example, all required but empty input elements get a red border or the message 'please scroll down' is presented.",
                "items": {
                  "type": "string",
                  "enum": [
                    "presentationIncomplete",
                    "responsesIncomplete"
                  ],
                  "x-parser-schema-id": "<anonymous-schema-40>"
                },
                "x-parser-schema-id": "<anonymous-schema-39>"
              }
            },
            "required": [
              "sessionId"
            ],
            "x-parser-schema-id": "<anonymous-schema-38>"
          }
        }
      }
    },
    "vopPlayerConfigChangedNotification": {
      "publish": {
        "operationId": "vopPlayerConfigChangedNotification",
        "summary": "Player Config Changed Notification",
        "description": "The host wants the player to update its configuration given at start.",
        "message": {
          "name": "vopPlayerConfigChangedNotification",
          "payload": {
            "type": "object",
            "properties": {
              "sessionId": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.sessionId",
              "playerConfig": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.playerConfig"
            },
            "required": [
              "sessionId",
              "playerConfig"
            ],
            "x-parser-schema-id": "<anonymous-schema-41>"
          }
        }
      }
    },
    "vopRuntimeErrorNotification": {
      "subscribe": {
        "operationId": "vopRuntimeErrorNotification",
        "summary": "Runtime Error Notification",
        "description": "The player had some trouble and the response process is at risk.",
        "message": {
          "name": "vopRuntimeErrorNotification",
          "payload": {
            "type": "object",
            "properties": {
              "sessionId": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.sessionId",
              "code": {
                "type": "string",
                "description": "The list should be defined after getting some experience with that operation.",
                "examples": [
                  "AUDIO_CORRUPT",
                  "GEOGEBRA_CRASH"
                ],
                "x-parser-schema-id": "<anonymous-schema-43>"
              },
              "message": {
                "type": "string",
                "description": "Some additional information.",
                "example": "Was not able to play Element 'audio_4'",
                "x-parser-schema-id": "<anonymous-schema-44>"
              }
            },
            "required": [
              "sessionId",
              "code"
            ],
            "x-parser-schema-id": "<anonymous-schema-42>"
          }
        }
      }
    },
    "vopWidgetCall": {
      "subscribe": {
        "operationId": "vopWidgetCall",
        "summary": "Request widget dialog",
        "description": "The player calls for the execution of a widget.",
        "message": {
          "name": "vopWidgetCall",
          "payload": {
            "type": "object",
            "properties": {
              "sessionId": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.sessionId",
              "callId": {
                "type": "string",
                "description": "Helps to assign the return value to an player element.",
                "x-parser-schema-id": "<anonymous-schema-46>"
              },
              "widgetType": {
                "type": "string",
                "enum": [
                  "WIDGET_CALC",
                  "WIDGET_PERIODIC_TABLE",
                  "WIDGET_MOLECULE_EDITOR",
                  "UNIT"
                ],
                "x-parser-schema-id": "<anonymous-schema-47>"
              },
              "parameters": {
                "type": "array",
                "items": {
                  "type": "object",
                  "description": "This object structure is used to send parameters to a widget.",
                  "properties": {
                    "key": {
                      "description": "Identifier for the parameter.",
                      "type": "string",
                      "minLength": 2,
                      "x-parser-schema-id": "<anonymous-schema-49>"
                    },
                    "value": {
                      "description": "Value of the parameter.",
                      "type": "string",
                      "x-parser-schema-id": "<anonymous-schema-50>"
                    }
                  },
                  "required": [
                    "key"
                  ],
                  "example": {
                    "key": "LAYOUT",
                    "value": "EXTENDED"
                  },
                  "x-parser-schema-id": "WidgetParameter"
                },
                "x-parser-schema-id": "<anonymous-schema-48>"
              },
              "state": {
                "type": "string",
                "format": "byte",
                "x-parser-schema-id": "<anonymous-schema-51>"
              }
            },
            "required": [
              "sessionId",
              "type"
            ],
            "x-parser-schema-id": "<anonymous-schema-45>"
          }
        }
      }
    },
    "vopWidgetReturn": {
      "publish": {
        "operationId": "vopWidgetReturn",
        "description": "Sends the return value of a widget call to the player.",
        "message": {
          "name": "vopWidgetReturn",
          "payload": {
            "type": "object",
            "properties": {
              "sessionId": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.sessionId",
              "callId": {
                "type": "string",
                "description": "Helps to find the corresponding element in the player.",
                "x-parser-schema-id": "<anonymous-schema-53>"
              },
              "state": {
                "type": "string",
                "format": "byte",
                "x-parser-schema-id": "<anonymous-schema-54>"
              }
            },
            "required": [
              "sessionId"
            ],
            "x-parser-schema-id": "<anonymous-schema-52>"
          }
        }
      }
    },
    "vopWindowFocusChangedNotification": {
      "subscribe": {
        "operationId": "vopWindowFocusChangedNotification",
        "summary": "Window Focus Changed Notification",
        "description": "In some use cases like exam mode the host must know whether the host or the player has the window's focus. If none of these windows have the focus (i. e. the testee has left the test), some logging or warning could be necessary. The player should listen to the window events 'blur' and 'focus'.",
        "message": {
          "name": "vopWindowFocusChangedNotification",
          "payload": {
            "type": "object",
            "properties": {
              "timeStamp": {
                "type": "string",
                "format": "date-time",
                "description": "Ensures, that later arriving states are ignored.",
                "example": "2021-11-15T10:22:25Z",
                "x-parser-schema-id": "<anonymous-schema-56>"
              },
              "hasFocus": {
                "type": "boolean",
                "description": "true if the player got the focus, false if it lost the focus.",
                "x-parser-schema-id": "<anonymous-schema-57>"
              }
            },
            "required": [
              "timeStamp",
              "hasFocus"
            ],
            "x-parser-schema-id": "<anonymous-schema-55>"
          }
        }
      }
    }
  },
  "components": {
    "schemas": {
      "LogEntry": "$ref:$.channels.vopStateChangedNotification.subscribe.message.payload.properties.log.items",
      "WidgetParameter": "$ref:$.channels.vopWidgetCall.subscribe.message.payload.properties.parameters.items",
      "SharedParameter": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.playerConfig.properties.sharedParameters.items",
      "PlayerState": "$ref:$.channels.vopStateChangedNotification.subscribe.message.payload.properties.playerState",
      "UnitState": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.unitState",
      "NavigationTarget": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.playerConfig.properties.enabledNavigationTargets.items",
      "SessionIdString": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.sessionId",
      "PlayerConfig": "$ref:$.channels.vopStartCommand.publish.message.payload.properties.playerConfig"
    }
  },
  "x-parser-spec-parsed": true,
  "x-parser-api-version": 3,
  "x-parser-spec-stringified": true
};
    const config = {"show":{"sidebar":true},"sidebar":{"showOperations":"byDefault"},"showOperations":true};
    const appRoot = document.getElementById('root');
    AsyncApiStandalone.render(
        { schema, config, }, appRoot
    );
  